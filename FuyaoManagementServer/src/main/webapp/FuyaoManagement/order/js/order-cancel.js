...
function findById(id){
	var data = {"status": "CANCEL", "orderId": id, "start": 0};
	store.get('proxy').set('url',requestIP + "/FuyaoManagementServer/order/find");
	store.load(data);
}

function findByName(name){
	var data = {"status": "CANCEL", "receiver": name, "start": 0};
	store.get('proxy').set('url',requestIP + "/FuyaoManagementServer/order/find");
	store.load(data);
}

function findByTel(tel){
	var data = {"status": "CANCEL", "tel": tel, "start": 0};
	store.get('proxy').set('url',requestIP + "/FuyaoManagementServer/order/find");
	store.load(data);
}

function findByDate(startDate, endDate){
	var data = {"status": "CANCEL", "startDate": startDate, "endDate": endDate, "start": 0};
	store.get('proxy').set('url',requestIP + "/FuyaoManagementServer/order/find");
	store.load(data);
}

function resetLoad(){
	var data = {"status": "CANCEL","start": 0};
