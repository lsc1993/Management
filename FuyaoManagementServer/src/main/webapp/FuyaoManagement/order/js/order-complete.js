...
function findById(id){
	var data = {"status": "COMPLETE", "orderId": id, "start": 0};
	store.get('proxy').set('url',requestIP + "/FuyaoManagementServer/order/find");
	store.load(data);
}

function findByName(name){
	var data = {"status": "COMPLETE", "receiver": name, "start": 0};
	store.get('proxy').set('url',requestIP + "/FuyaoManagementServer/order/find");
	store.load(data);
}

function findByTel(tel){
	var data = {"status": "COMPLETE", "tel": tel, "start": 0};
	store.get('proxy').set('url',requestIP + "/FuyaoManagementServer/order/find");
	store.load(data);
}

function findByDate(startDate, endDate){
	var data = {"status": "COMPLETE", "startDate": startDate, "endDate": endDate, "start": 0};
	store.get('proxy').set('url',requestIP + "/FuyaoManagementServer/order/find");
	store.load(data);
}

function resetLoad(){
	var data = {"status": "COMPLETE","start": 0};
